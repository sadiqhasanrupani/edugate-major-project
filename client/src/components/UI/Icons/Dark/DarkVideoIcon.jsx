import React from "react";

const DarkVideoIcon = () => {
  return (
    <svg
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
    >
      <path
        opacity="0.4"
        d="M13 3.25H7C3.58 3.25 2.25 4.58 2.25 8V16C2.25 18.3 3.5 20.75 7 20.75H13C16.42 20.75 17.75 19.42 17.75 16V8C17.75 4.58 16.42 3.25 13 3.25Z"
        fill="white"
      />
      <path
        d="M11.5001 11.38C12.5384 11.38 13.3801 10.5383 13.3801 9.5C13.3801 8.4617 12.5384 7.62 11.5001 7.62C10.4618 7.62 9.62012 8.4617 9.62012 9.5C9.62012 10.5383 10.4618 11.38 11.5001 11.38Z"
        fill="white"
      />
      <path
        d="M21.65 6.17C21.24 5.96 20.38 5.72 19.21 6.54L17.73 7.58C17.74 7.72 17.75 7.85 17.75 8V16C17.75 16.15 17.73 16.28 17.73 16.42L19.21 17.46C19.83 17.9 20.37 18.04 20.8 18.04C21.17 18.04 21.46 17.94 21.65 17.84C22.06 17.63 22.75 17.06 22.75 15.63V8.38C22.75 6.95 22.06 6.38 21.65 6.17Z"
        fill="white"
      />
    </svg>
  );
};

export default DarkVideoIcon;
