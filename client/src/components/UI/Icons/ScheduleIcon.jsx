import React from "react";

const ScheduleIcon = () => {
  return (
    <svg
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <g id="vuesax/bulk/timer">
        <g id="vuesax/bulk/timer_2">
          <g id="timer">
            <path
              id="Vector"
              opacity="0.4"
              d="M12.0001 22C16.7884 22 20.6701 18.1183 20.6701 13.33C20.6701 8.5417 16.7884 4.66 12.0001 4.66C7.21177 4.66 3.33008 8.5417 3.33008 13.33C3.33008 18.1183 7.21177 22 12.0001 22Z"
              fill="#7959FD"
            />
            <path
              id="Vector_2"
              d="M12 13.75C11.59 13.75 11.25 13.41 11.25 13V8C11.25 7.59 11.59 7.25 12 7.25C12.41 7.25 12.75 7.59 12.75 8V13C12.75 13.41 12.41 13.75 12 13.75Z"
              fill="#7959FD"
            />
            <path
              id="Vector_3"
              d="M14.8901 3.45H9.11014C8.71014 3.45 8.39014 3.13 8.39014 2.73C8.39014 2.33 8.71014 2 9.11014 2H14.8901C15.2901 2 15.6101 2.32 15.6101 2.72C15.6101 3.12 15.2901 3.45 14.8901 3.45Z"
              fill="#7959FD"
            />
          </g>
        </g>
      </g>
    </svg>
  );
};

export default ScheduleIcon;
